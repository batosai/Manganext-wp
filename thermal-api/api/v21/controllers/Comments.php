...
class Comments {
	...
	public static function model() {
		if ( !isset( self::$_model ) ) {
			self::$_model = new \Voce\Thermal\v21\Models\Comments();
		}
		return self::$_model;
	}
	...
	protected static function convert_request( $request_args ) {
		...
		$request_filters = array(
			...
			'orderby' => array( ),
			'order' => array( ),
			'in' => array( '\\Voce\\Thermal\\v21\\toArray', '\\Voce\\Thermal\\v21\\applyInt' ),
			'parent' => array( '\\intval' ),
		...
		if ( !empty( $request_args['per_page'] ) ) {
			if ( absint( $request_args['per_page'] ) > \Voce\Thermal\v21\MAX_TERMS_PER_PAGE ) {
				$request_args['per_page'] = \Voce\Thermal\v21\MAX_COMMENTS_PER_PAGE;
			} else {
