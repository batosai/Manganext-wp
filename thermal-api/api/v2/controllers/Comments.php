...
class Comments {
	...
	public static function model() {
		if ( !isset( self::$_model ) ) {
			self::$_model = new \Voce\Thermal\v2\Models\Comments();
		}
		return self::$_model;
	}
	...
	protected static function convert_request( $request_args ) {
		...
		if ( !empty( $request_args['per_page'] ) ) {
			if ( absint( $request_args['per_page'] ) > \Voce\Thermal\v2\MAX_TERMS_PER_PAGE ) {
				$request_args['per_page'] = \Voce\Thermal\v2\MAX_COMMENTS_PER_PAGE;
			} else {
